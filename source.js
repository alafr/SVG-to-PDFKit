var SVGtoPDF = function(doc, svg, x, y, options) {
    ...
    function docUsePattern(pattern, stroke) {
        ...
        let ref = doc.ref({
            ...
            PaintType: 1,
            TilingType: 2,
            BBox: [pattern.x, pattern.y, pattern.dx, pattern.dy],
            XStep: pattern.dx,
            YStep: pattern.dy,
    ...
    function decodeEntities(str) {
        ...
        return (str.replace(/&(?:#([0-9]+)|#[xX]([0-9A-Fa-f]+)|([0-9A-Za-z]+));/g, function(mt, m0, m1, m2) {
            if (m0) { return String.fromCharCode(parseInt(m0, 10)); } else if (m1) { return String.fromCharCode(parseInt(m1, 16)); } else if (m2 && Entities[m2]) { return String.fromCharCode(Entities[m2]); } else { return mt; }
        }));
    }
    ...
    var SvgShape = function(commands) {
        let pathCommands = this.pathCommands = [],
            startX = 0,
            startY = 0,
            currX = 0,
            currY = 0,
            lastCom, lastCtrlX, lastCtrlY;
        ...
        this.resetCommands = function() {
                startX = startY = currX = currY = lastCtrlX = lastCtrlY = 0;
                lastCom = null;
                return this;
            }
            // Shape creation
        ...
        Object.defineProperty(this, 'pathSegments', {
            get: function() {
                ...
                for (let i = 0; i < pathCommands.length; i++) {
                    let command = pathCommands[i][0],
                        values = pathCommands[i].slice(3),
                        segment;
                    switch (command) {
                        case 'M':
                            segment = null;
                            startX = currX = values[0];
                            startY = currY = values[1];
                            break;
                        case 'L':
    ...
    var SvgElem = function(obj, inherits) {
        switch (obj.nodeName) {
            case 'use':
                if (this instanceof SvgElemUse) { break; } else { return new SvgElemUse(obj, inherits); }
            case 'symbol':
                if (this instanceof SvgElemSymbol) { break; } else { return new SvgElemSymbol(obj, inherits); }
            case 'g':
                if (this instanceof SvgElemGroup) { break; } else { return new SvgElemGroup(obj, inherits); }
            case 'a':
                if (this instanceof SvgElemLink) { break; } else { return new SvgElemLink(obj, inherits); }
            case 'svg':
                if (this instanceof SvgElemSvg) { break; } else { return new SvgElemSvg(obj, inherits); }
            case 'image':
                if (this instanceof SVGElemImage) { break; } else { return new SVGElemImage(obj, inherits); }
            case 'rect':
                if (this instanceof SvgElemRect) { break; } else { return new SvgElemRect(obj, inherits); }
            case 'circle':
                if (this instanceof SvgElemCircle) { break; } else { return new SvgElemCircle(obj, inherits); }
            case 'ellipse':
                if (this instanceof SvgElemEllipse) { break; } else { return new SvgElemEllipse(obj, inherits); }
            case 'line':
                if (this instanceof SvgElemLine) { break; } else { return new SvgElemLine(obj, inherits); }
            case 'polyline':
                if (this instanceof SvgElemPolyline) { break; } else { return new SvgElemPolyline(obj, inherits); }
            case 'polygon':
                if (this instanceof SvgElemPolygon) { break; } else { return new SvgElemPolygon(obj, inherits); }
            case 'path':
                if (this instanceof SvgElemPath) { break; } else { return new SvgElemPath(obj, inherits); }
            case 'text':
                if (this instanceof SvgElemText) { break; } else { return new SvgElemText(obj, inherits); }
            case 'tspan':
                if (this instanceof SvgElemTspan) { break; } else { return new SvgElemTspan(obj, inherits); }
            case 'textPath':
                if (this instanceof SvgElemTextPath) { break; } else { return new SvgElemTextPath(obj, inherits); }
            case '#text':
                if (this instanceof SvgElemTextNode) { break; } else { return new SvgElemTextNode(obj, inherits); }
        }
    ...
    var SvgElemSvg = function(obj, inherits) {
        ...
        this.getTransformation = function() {
            return multiplyMatrix(
                this.get('transform'), [1, 0, 0, 1, x, y],
                parseAspectRatio(aspectRatio, width, height, viewBox[2], viewBox[3]), [1, 0, 0, 1, -viewBox[0], -viewBox[1]]
            );
        };
    };
        ...
        this.getPaint = function(bBox, gOpacity, isClip, isMask) {
            ...
            let viewBox = this.getViewbox('viewBox', [0, 0, width, height]),
                ...
                aspectRatioMatrix = multiplyMatrix(
                    parseAspectRatio(aspectRatio, width, height, viewBox[2], viewBox[3], 0), [1, 0, 0, 1, -viewBox[0], -viewBox[1]]
                ),
    ...
    var SvgElemRect = function(obj, inherits) {
        ...
        if (rx === undefined && ry === undefined) { rx = ry = 0; } else if (rx === undefined && ry !== undefined) { rx = ry; } else if (rx !== undefined && ry === undefined) { ry = rx; }
        if (w > 0 && h > 0) {
            if (rx && ry) {
    ...
    var SvgElemText = function(obj, inherits) {
        ...
        (function(textParentElem) {
            let processedText = '',
                ...
                textPaths = [],
                currentChunk = [],
                currentAnchor, currentDirection, currentX = 0,
                currentY = 0;
